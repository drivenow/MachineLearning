...
tb = TensorBoard("D:/data/mylog/KerasDL/board/mnist_cnn",write_graph=True)
...
info = model.fit(X_train[:,:,:,:], Y_train[:,:], batch_size=batch_size, nb_epoch=nb_epoch,
          verbose=1, validation_data=(X_test, Y_test),callbacks=[lrate, tb])
score = model.evaluate(X_test[0], Y_test[0], verbose=0)
print('Test score:', score[0])
#print('Test accuracy:', score[1])
#print(info.history)

#%%
from mykeras.grid.classify import grid_search_classify
...
grid_result = grid_search_classify(cnn_model,X_train,Y_train,2,param_grid,cv=3)
